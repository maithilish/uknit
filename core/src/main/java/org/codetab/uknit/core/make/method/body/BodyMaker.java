...
public class BodyMaker {
     ...
     * varStmt.addStmts(methodDecl, stmts); }
     *
     * public void generateWhenStmts(final MethodDeclaration methodDecl, final
     * Heap heap) { List<Statement> stmts = whenStmt.createStmts(heap);
     ...
     */
    /*
     * public void generateCallStmt(final MethodDeclaration methodDecl, final
     * Heap heap) { Statement stmt = callStmt.createStmts(heap);
     * callStmt.addStmt(methodDecl, stmt); }
     *
     * public void generateAssertStmt(final MethodDeclaration methodDecl, final
     * Heap heap) { Optional<Statement> stmt = assertStmt.createStmt(heap);
     * assertStmt.addStmt(methodDecl, stmt); }
     *
     * public void generateFailAssertStmt(final MethodDeclaration methodDecl,
     * final Heap heap) { // when no assert or verify statement exists, generate
     * fail assertion if (!heap.isAsserted()) { Optional<Statement> stmt =
     * assertStmt.createFailStmt(heap); assertStmt.addStmt(methodDecl, stmt); }
     * }
     *
     * public void generateVerifyStmts(final MethodDeclaration methodDecl, final
     * Heap heap) { List<Statement> stmts = verifyStmt.createStmts(heap);
