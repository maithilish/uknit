...
public class Inserters {
     ...
     * @return
     */
    // public IVar getKeyArg(final IVar collectionVar, final MethodInvocation
    // mi,
    ...
    public List<Invoke> findAllowedPacks(final IVar var, final Class<?> clz,
        ...
        List<Invoke> allowedInvokes =
                invokes.stream().filter(i -> isInsertable(var, clz, i.getExp()))
                        .collect(Collectors.toList());
     ...
     */
    // REVIEW
    public Optional<IVar> findPutInferVar(final List<Invoke> expVars,
            final IVar var, final Heap heap) {
        List<Type> typeArgs = insertVars.getTypeArgs(var.getType());

        for (Invoke expVar : expVars) {
            IVar leftVarO = expVar.getVar();
            if (nonNull(leftVarO)) {
                Type type = leftVarO.getType();
                if (typeArgs.stream().anyMatch(
                        ta -> ta.toString().equals(type.toString()))) {
                    return Optional.of(leftVarO);
                }
            }
        }
        return Optional.empty();
